...
<div class="eventorganiser-event-meta news-item-top">
	...
	<div class="eo-event-meta">
		...
		<?php if ( get_the_terms( get_the_ID(), 'event-category' ) ) { ?>
			<h5><strong><?php esc_html_e( 'Categories', 'eventorganiser' ); ?>:</strong> <?php echo get_the_term_list( get_the_ID(),'event-category', '', ', ', '' ); ?></h5>
		<?php } ?>

		<?php if ( ! eo_recurs() ) { ?>
			<!-- Single event -->
			<small><?php esc_html_e( '', 'eventorganiser' );?><?php echo eo_format_event_occurrence();?>
		...
		<?php if ( eo_get_venue() ) {
			...
			<h5><strong><?php echo esc_html( $tax->labels->singular_name ) ?>:</strong> <a href="<?php eo_venue_link(); ?>"> <?php eo_venue_name(); ?></a></h5>
		<?php } ?>

		

		<?php if ( get_the_terms( get_the_ID(), 'event-tag' ) && ! is_wp_error( get_the_terms( get_the_ID(), 'event-tag' ) ) ) { ?>
			<h5><strong><?php esc_html_e( 'Tags', 'eventorganiser' ); ?>:</strong> <?php echo get_the_term_list( get_the_ID(), 'event-tag', '', ', ', '' ); ?></h5>
		<?php } ?>
